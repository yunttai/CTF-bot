...
- 로컬에서 처음 DB를 만들면 모든 항목이 `discord_notified = 0`으로 들어가므로, 첫 알림 실행 때 아직 안 보낸 CTF들이 전송됩니다.
...
```bash
.\.venv\Scripts\python -m ctf_bot.notifier --current-db data/ctf_snapshot.db --dry-run
```
